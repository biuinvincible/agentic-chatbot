"""Utility functions and helpers for the Local Deep Research agent."""
...
from agents.deep_research_agent.state import ResearchComplete, Summary
...
async def tavily_search(
    ...
    summarization_model = ChatGoogleGenerativeAI(
        model=configurable.summarization_model.replace("google:", ""),
        temperature=0,
...
async def tavily_search_async(
    ...
    """Execute multiple Tavily search queries asynchronously with retry logic.
    ...
    max_retries = getattr(configurable, 'max_api_retries', 3) if configurable else 3
    base_delay = getattr(configurable, 'api_request_delay', 1.0) if configurable else 1.0
            ...
            try:
                ...
                if "quota" in error_str or "limit" in error_str or "429" in error_str:
                    if attempt < max_retries:
                        ...
                        delay = min(base_delay * (2 ** attempt) + (attempt * 0.1), 10.0)  # Max 10s delay
                        logger.warning(f"Rate limit hit for query '{query[:50]}...', retrying in {delay:.1f}s (attempt {attempt + 1}/{max_retries + 1})")
                        await asyncio.sleep(delay)
                        continue
                    else:
                        logger.error(f"Max retries exceeded for query '{query[:50]}...': {e}")
                        # Return empty result instead of raising to avoid failing the entire research
                        return {"query": query, "results": [], "error": str(e)}
                else:
                    # Not a rate limit error, log and return empty result to avoid failing the entire research
                    logger.error(f"Non-rate-limit error for query '{query[:50]}...': {e}")
                    return {"query": query, "results": [], "error": str(e)}
    
    # Create search tasks with retry logic
    search_tasks = [
        search_with_retry(query)
...
##########################
# Web Scraping Utils with Crawl4AI
##########################

async def scrape_webpage(url: str) -> str:
    """Scrape the full content of a webpage using Crawl4AI.
    ...
    if not CRAWL4AI_AVAILABLE:
        return f"Error: Crawl4AI not available for scraping {url}"
...
async def scrape_multiple_webpages(urls: List[str]) -> Dict[str, str]:
    """Scrape multiple webpages in parallel using Crawl4AI.
    ...
    """
    if not CRAWL4AI_AVAILABLE:
        return {url: f"Error: Crawl4AI not available for scraping {url}" for url in urls}
